 ...
 * 한개의 router 에서는 반드시 Response 를 한번만 해야한다
 *
 * router 에서 if 조건문을 사용하거나, try 처리를 하는 과정에서
 * res.send(),res.write(), res.end(), res.redirect(), res.render()
 * 이러한 코드가 중복되서 발견될 경우 nodejs express 에서는 경고성
 * 오류를 발생한다.
 * 경고성 오류는 그냥 무시되기도 하지만, 간혹 프로젝트를 멈추는 경우가 있다
 * 중목된 Response 코드가 있을 경우
 * 예기치 못하게 중복된 Response 코드가 실행되는 경우가 있기 때문이다.
...
router.post("/insert", fileUp.array("b_upfile"), async (req, res) => {
  ...
  try {
     ...
     * 3. tbl_files 데이블에 insert 할때 사용한다
     */
...
router.get("/delete/:seq", async (req, res) => {
  ...
  await files.forEach(async (file) => {
    try {
      ...
      // 파일이 어떤 상태인가를 검사하는 함수
      // 만약 delFile 이 없으면 exception이 발생
      fs.statSync(delFile);
