...
/*
AUTO_INCREMENT 속성
INSERT 를 수행할떄 특별히 값이 없을 때 또는 0일때
	기존의 칼럼 값을 비교하여 1 만큼 자동 증가된 값으로 지정한다
DEFAULT 속성
INSERT 를 수행할때 특별히 값이 없을때 채워넣을 값 지정

(DATE_FORMAT(NOW(),'%Y-%m-%d')
현재 DB가 설치된 서버(컴퓨터)의 현재 시각을 가져와서
YYYY-MM-DD 형식의 문자열로 변화하라

NOT NULL
DEFAULT CURRENT_TIMESTAMP
ON UPDATE CURRENT_TIMESTAMP

최초 INSERT 가 될떄 자동으로 현재 TIMESTAMP(시간 일렬번호)를
날짜시각 형태로 변경하여 저장하고
...
tbl_bbs 테이블의 b_seq 칼럼과 현재 table의 f_bseq 칼럼을
...
해당하는 데이터는 삭제, 키값의 Udate 는 기본적으로 금지된다

ON DELETE CASCADE
tbl_bbs 의 어떤 데이터를 삭제를 시도하는 경우
그 키값에 해당하는 데이터가  tbl_files 에 있으면 같이 모두 삭제
...
1:N 관계 table 에서 N 의 table에 설정한다
...
-- 단 이때는 두 table 간에 EQ JOIN 이 성립되어야 하다
...
-- ON DELETE CASACDE 가 설정되어 있기 때문에
