...
router.post("/insert", (req, res) => {
  ...
  mysql.execute(sql, Object.values(student), (err, result, field) => {
     ...
     * 보여줘야 한다
     *
     * 이미 학생정보 List를 보여주는 router 를 만들어 두었다
     * 다시 여기에서 list SELECT 하여 보여주는 코드를
     * 작성하는 대신
     * server 의 router 에서 web browser에게 요청을 한다
...
router.get("/:st_num/detail", (req, res) => {
   ...
   * 명령이 실행된다
   * 만약 DELETE, UPDATE 명령을 수해할때 이러한 코드를
   * 작성혀면 해커에 의해 DB 가 바로 손상될수 있다
   *
...
router.post("/:st_num/update", (req, res) => {
  ...
  console.log({ st_num, st_name, st_dept, st_grade, st_tel, st_addr });
  const sql =
