...
router.get("/detail/:bcode", async (req, res) => {
  ...
  try {
     ...
     * findoOne() 함수는 1개의 결과만 찾고 만약 결과가 여러개 이더라도
     *    최초의 한개만 추출한다
     *    결과는 무조건 단일 객체이다
     */
 ...
 * B0112코드를 자동생성하여 response 하기
 *
 */
router.get("/get/bcode", async (req, res) => {
  /**
   * sequelize 에서는 기본적인 CRUD를 함수로 제공한다
   * 하지만 기본적으로 제공하는 기능이 아닌 특별한 SQL 을 사용하는 방법
   * 이때 기본이외의 별도 SQL을 작성하는 것을 Raw SQL Query 라고 한다
   */
  ...
  try {
    ...
    // bcode 값을 0번째 부터 1번째 앞까지 자르기 : B 문자열 한개를 추출하기
